...
Metrics.prototype.schedule = function () {
    ...
    this.__handler = setInterval(function () {
        ...
        log.info('sample#node.rss=' + memory.rss, 'sample#node.heapTotal=' + memory.heapTotal, 'sample#node.heapUsed=' + memory.heapUsed);
        var load = os.loadavg();
        log.info('sample#os.load1=' + load[0], 'sample#os.load5=' + load[1], 'sample#os.load15=' + load[2]);
        var bench = new Hoek.Bench();
        setImmediate(function () {
            log.info('sample#node.threadDelay=' + bench.elapsed());
        });
