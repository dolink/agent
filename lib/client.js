...
Client.prototype.activate = function () {
    ...
    url += '/rest/v0/node/' + self.creds.serial + '/activate';
            ...
            if (response.statusCode == 200) {
                if (body) {
                    ...
                    if (ret.data) {
                        ...
                        creds.token = ret.data.token;

                        d.resolve(ret.data);
                    } else {
                        d.reject(new Error(ret.error || 'Failed activating'));
                    }
                } else {
...
Client.prototype.initialize = function () {
    ...
    function flushBuffer() {
        ...
        if (self.sendBuffer.length > 0) {
            ...
            var topic = path.join('$cloud', self.creds.serial, 'data');
...
Client.prototype.down = function (a1, a2, a3) {
    this.connected = false;
...
Client.prototype.subscribe = function () {
    ...
    this.router.subscribe(path.join('$node', serial, 'revoke'), function revokeCredentials() {
    ...
    this.router.subscribe(path.join('$node', serial, 'commands'), {qos: 2}, function execute(topic, cmd) {
        log.info('MQTT execute', cmd);
        self.handlers.commands(cmd);
    });

    this.router.subscribe(path.join('$node', serial, 'update'), {qos: 2}, function update(topic, cmd) {
        log.info('MQTT update', cmd);
        // TODO update
    });

    this.router.subscribe(path.join('$node', serial, 'config'), {qos: 2}, function config(topic, cmd) {
...
Client.prototype.sendData = function sendData(data) {
    ...
    if (this.connected && this.mqttclient) {
        ...
        var topic = path.join('$cloud', nodeId, 'devices', deviceId, 'data');
...
Client.prototype.sendConfig = function sendConfig(id, responses) {
    ...
    if (this.connected && this.mqttclient) {
        ...
        if (id) msg.id = id;
        var topic = path.join('$cloud', this.creds.serial, 'config');
...
Client.prototype.sendHeartbeat = function sendHeartbeat(data) {
    ...
    if (this.connected && this.mqttclient) {
        ...
        var nodeId = this.creds.serial;
        var deviceId = [data.G, data.V, data.D].join('_');
        var topic = path.join('$cloud', nodeId, 'devices', deviceId, 'heartbeat');
...
//    // if we have been told we don't exist don't continue trying..
//    if (err.message.match(/Bad username or password/)) {
//
//        // end the client
//        this.mqttclient.end();
//
//        // either way we are going to restart in 30 seconds
//        restart(30000);
//
//        this.creds.token = '';
//
...
//
//        // Attempt to save an empty token and restart
//        this.creds.saveToken(this.creds);
//        restart(3000);
//    }
};
